...
import SeedrandomUtils from 'module/utils/SeedrandomUtils';
...
/**
 * 基础的移动的表格模型,用于移动的3D模型
 * 记录移动的模型的表格位置、表格的最后位置，以及3D模型的最后位置
 */
class BaseMoveTableModel extends  Object3D{
    ...
    _updateTablePosition() {
        // 获取表格位置的值
        SeedrandomUtils.getTablePosition(this.position, this.parent.tableX, this.parent.tableY, this.tablePosition);
        ...
        //上一个块
        if ( this.previousChunk !== context && context !== node){
            //最后的位置
